...
class PINN:
    ...
    def loss_res(self, x):
        ...
        r_norm1 = self.eta * torch.abs(f_1) / torch.max(torch.abs(f_1))
        r_norm2 = self.eta * torch.abs(f_2) / torch.max(torch.abs(f_2))
        self.rsum1 = (self.rsum1 * self.gamma + r_norm1).detach()
        self.rsum2 = (self.rsum2 * self.gamma + r_norm2).detach()

        loss = torch.mean((self.rsum1 * f_1) ** 2 + (self.rsum2 * f_2) ** 2)
...
if __name__ == "__main__":
    ...
    # model save
    torch.save(pinn.model.state_dict(), 'RBA.pth')
